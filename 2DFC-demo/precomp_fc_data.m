...
% modified and simplified for my purposes

function [Q, A] =  ...
    precomp_fc_data(d, C, Z, E, n_over, modes_to_reduce, num_digits, n_r)
...
hr = sym(h0/n_r);
continuation_r = sym(x0+h0*(d-1) + hr*(1:n_r*C).');
...
A = vpa([cos(2*sym(pi)*continuation_r * k_cos  / ((N_coarse - 1) * h0)),...
        sin(2*sym(pi)*continuation_r * k_sin  / ((N_coarse - 1) * h0))] * Coeffs);
