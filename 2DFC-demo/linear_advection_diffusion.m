...
% Compute flow field by with Spectral differentiation
[u_x, u_y] = R_p.grad(R_p.f_R);

%% Linear Advection-Diffusion
% time step size computed with Von-Neumann stability analysis
dt = min(1.4*Pe/pi^2*h^2, 1.5/(pi*max(sqrt(u_x.^2+u_y.^2), [], 'all'))*h)
...
for t = t_mesh
    ...
    scatter(R_c.R_X(R_c.in_interior), R_c.R_Y(R_c.in_interior), 50, f_np1(R_c.in_interior), 'filled'); % Color by z values
    caxis([0, 1])
    colorbar; % Show color scale
    xlabel('x'); ylabel('y'); title('Concentration vs Time');
    drawnow;
