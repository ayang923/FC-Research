...
C2_patch = construct_C2_patch(f, 0.4, 2*pi-0.4, 0, n_C2, n_C2, d, eps_xi_eta, eps_xy); % data associated with patch

window_patch_xi = construct_S_patch(f, 0.1, 0.5, h_S, n_S_h, d, eps_xi_eta, eps_xy);
window_patch_eta = construct_S_patch(f, 2*pi-0.1, 2*pi-0.5, h_S, n_S_h, d, eps_xi_eta, eps_xy);
S_patch = construct_S_patch(f, 0.5, 2*pi-0.5, h_S, n_S_w, d, eps_xi_eta, eps_xy);
...
function C2_patch = construct_C2_patch(f, theta_A, theta_B, theta_C, n_xi, n_eta, d, eps_xi_eta, eps_xy)
    l_theta = @(theta) [2*sin(theta/2), -sin(theta)];
    ...
    C2_patch = C2_patch_obj(M_p, J, eps_xi_eta, eps_xy, n_xi, n_eta, d, nan, nan); % data associated with patch    
    
    [X_W, Y_W] = C2_patch.W.xy_mesh;
    C2_patch.W.f_XY = f(X_W, Y_W);
    [X_L, Y_L] = C2_patch.L.xy_mesh;
    C2_patch.L.f_XY = f(X_L, Y_L);
end
