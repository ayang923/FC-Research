...
interior_patches =  construct_interior_patches(curve_seq, R.h , M, eps_xi_eta, eps_xy);
figure;
hold on;
for i = 1:curve_seq.n_curves
...
for i = 1:curve_seq.n_curves
    %S_patch
    Q_interior_patch = interior_patches{i};
    [bound_X, bound_Y] = Q_interior_patch.boundary_mesh_xy(n_r, false);
...
function interior_patches = construct_interior_patches(curve_seq, h_norm, M, eps_xi_eta, eps_xy)
    interior_patches = cell(curve_seq.n_curves, 1);
    prev = curve_seq.last_curve;
    curr = curve_seq.first_curve;
    for i = 1:curve_seq.n_curves
...
%             %solve for if normals cross boundary
%             [theta_1_prev, ~] = compute_normal_intersection(curr, prev, M, h_norm, eps_xy, [0; 1])
%             [theta_1_next, ~] = compute_normal_intersection(curr, curr.next_curve, M, h_norm, eps_xy, [1; 0])
